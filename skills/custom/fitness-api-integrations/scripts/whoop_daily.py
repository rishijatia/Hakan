#!/usr/bin/env python3
"""
WHOOP Daily Profile Updater
Refreshes OAuth token, fetches data, writes human-readable summary.

Usage:
    python3 whoop_daily.py              # Fetch + print summary
    python3 whoop_daily.py --refresh    # Just refresh token
    
Requires: /opt/data/whoop/tokens.json with:
    {
        "access_token": "...",
        "refresh_token": "...",
        "client_id": "...",
...
Output: /opt/data/whoop/daily_profile.json (markdown summary)
        /opt/data/whoop/daily_profile_raw.json (raw API data)
"""
...
def api_get(path, token, retries=3):
    """GET request with retry, timeout, and User-Agent."""
    import urllib.request
...
def fetch_all(token):
    """Fetch latest data from all endpoints. Returns None on auth error."""
    data = {}
...
def build_summary(data):
    """Build a human-readable daily profile in markdown."""
...
def main():
    if "--refresh" in sys.argv:
        tokens = load_tokens()
        result = refresh_access_token(tokens)
        if result:
        ...
        if not new_token:
            error_profile = {
                ...
                "auth_url": (
                    ...
                    f"&client_id={tokens['client_id']}"
                    f"&redirect_uri={tokens['redirect_uri']}"
                    f"&scope=read:recovery+read:sleep+read:workout+read:cycles"
                    f"+read:profile+read:body_measurement+offline"
                    f"&state=rereauth01"
                ),
            }
    ...
    # Save raw data
    raw_file = str(PROFILE_FILE).replace(".json", "_raw.json")
